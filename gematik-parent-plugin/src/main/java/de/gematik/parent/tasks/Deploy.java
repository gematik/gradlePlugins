...
public class Deploy extends AbstractGematikTask {
    ...
    public Deploy() {
        setGroup("Gematik");
        setDescription("Build the Project and publish artefacts to mavenlocal and deploy to remote repository wiht 'uploadArchives'.");
        dependsOn(getProject().getTasks().findByName("build"), getProject().getTasks().findByName("publishToMavenLocal"));
        finalizedBy(getProject().getTasks().findByName("uploadArchives"));
    }
    ...
    private void printTaskInfo() {
        ...
        getLogger().quiet(getLineForString("   Build: publishToMavenLocal => uploadArchives      "));
        printTextBlockBorder();
    }
